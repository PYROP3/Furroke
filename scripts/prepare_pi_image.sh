...
read -p "This will clean a patake install to make it ready for image deployment. Proceed? (y/n): " REPLY
if [ $REPLY = "y" ]
 then
...
echo "*** Double-checking /etc/rc.local expecting to see a line about launching patake:"
cat /etc/rc.local | grep patake

echo "*** Double-checking we have a wpa_supplicant example file in /boot"
ls -la /boot/wpa_supplicant.conf.example

echo "*** Expecting hostname to be 'patake'"
hostname
