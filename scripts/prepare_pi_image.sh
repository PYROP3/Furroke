...
read -p "This will clean a Furroke install to make it ready for image deployment. Proceed? (y/n): " REPLY
if [ $REPLY = "y" ]
 then
...
echo "*** Double-checking /etc/rc.local expecting to see a line about launching Furroke:"
cat /etc/rc.local | grep Furroke

echo "*** Double-checking we have a wpa_supplicant example file in /boot"
ls -la /boot/wpa_supplicant.conf.example

echo "*** Expecting hostname to be 'Furroke'"
hostname
