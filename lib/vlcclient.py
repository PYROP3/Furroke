...
class VLCClient:
    def __init__(self, port=5002, path=None, qrcode=None, url=None):
        ...
        if self.platform == "windows":
            self.tmp_dir = os.path.expanduser(r"~\\AppData\\Local\\Temp\\Furroke\\")
        else:
            self.tmp_dir = "/tmp/Furroke/"
    ...
    def get_marquee_cmd(self):
        return ["--sub-source", 'logo{file=%s,position=9,x=2,opacity=200}:marq{marquee="Furroke - connect at: \n%s",position=9,x=38,color=0xFFFFFF,size=11,opacity=200}' % (self.qrcode, self.url)]
